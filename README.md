# ІСтаТП · ЛР1 «Месенджер» (ASP.NET Core MVC) — гілка етапу 1.7

**Виконавець:** Костащук Ярослав Васильович, група К26 · **Викладач:** Панченко Т. В.

Ця гілка показує стан проєкту на **етапі 1.7**. Нижче — що додано або змінено на кожному етапі (до цього включно).
...
- **Етап 1.5** — додано графік кількості повідомлень за чатами (Chart.js, `ChartController`).
- **Етап 1.6** — додано експорт та імпорт повідомлень у форматі Excel (ClosedXML) у `MessagesController`.
- **Етап 1.7** — додано автентифікацію, авторизацію та ролі Admin/User (ASP.NET Identity): вхід і реєстрація за іменем користувача, захист сторінок `[Authorize]`.  ◀ **цей етап (гілка)**
